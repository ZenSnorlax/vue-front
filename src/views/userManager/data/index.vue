...
import { getUsersAdd } from "@/api/user";
...
// 动态获取数据并创建图表
const fetchUserAddData = async (selectedYear: number) => {
  try {
    const response = await getUsersAdd({ year: selectedYear });
    const { months, counts } = response.data;

    const incomeChartOption: EChartsOption = {
      title: {
        text: `${selectedYear} 新增用户`,
        left: "center",
        textStyle: {
          fontSize: 18,
          fontWeight: "bold",
    ...
    console.error(`获取 ${selectedYear} 年新增用户数据失败:`, error);
  }
};
...
const handleChange = (newYear: number) => {
  fetchUserAddData(newYear);
};

// 在组件挂载时加载默认年份数据
onMounted(() => {
  fetchUserAddData(year.value);
});
</script>
